...
while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                ...
                if show.collidepoint(mouse_pos) and show_button == False:
                    ...
                    cookie_scaled = pygame.transform.scale(cookie, (150, 150))
                    clicks += 1 + upgradex
                elif upgrade1.collidepoint(mouse_pos):
    ...
    def text():
        ...
        button_text_rect.center = (35, 20)  # changed x-coordinate

    def upgrades():
        global upgrade1, upgrade2, show
        show = pygame.draw.rect(
            screen, (0, 200, 250), (2, 2, 70, 40))

        if show_button == True:
            if clicks >= upgrade1_price:
                upgrade1 = pygame.draw.rect(
                    screen, (0, 200, 0), (15, 265, 70, 50))
            else:
                upgrade1 = pygame.draw.rect(
                    screen, (200, 0, 0), (15, 265, 70, 50))
            upgrade1_text = font.render(
                f"Upgrade 1 ({upgrade1_price})", True, (0, 0, 0))
            upgrade1_text_rect = upgrade1_text.get_rect()
            upgrade1_text_rect.center = (35, 70)  # changed x-coordinate

            if clicks >= upgrade2_price:
                upgrade2 = pygame.draw.rect(
                    screen, (0, 200, 0), (100, 265, 70, 50))
            else:
                upgrade2 = pygame.draw.rect(
                    screen, (200, 0, 0), (100, 265, 70, 50))
            upgrade2_text = font.render(
                f"Upgrade 2 ({upgrade2_price})", True, (0, 0, 0))
            upgrade2_text_rect = upgrade2_text.get_rect()
            upgrade2_text_rect.center = (35, 150)  # changed x-coordinate

        def texts():
            global up_text, up, up_text2, up_text2_rect, up_text_rect, upgrade_text, upgrade_text_rect, upgrade_text2, upgrade_text2_rect, show_text, show_text_rect
